...
- 1本目（001_sns_investment_scam）のカット密度：全11シーンでcut_pacing警告
  ...
  **【撤回】下記「方針転換：SCENE 002〜011のCUT形式移行」を参照。この決定は撤回済み**
...
## 追記：方針転換（SCENE 002〜011のCUT形式移行）

### 背景
本ADR確定当初は「SCENE 001のみカット分割、SCENE 002〜011は1画像/シーンのままDaVinci編集
（Ken Burns効果等）で吸収する」との判断だった（上記「結果」参照、撤回済み）。しかし
クライアントワークの実務基準では20分動画あたり80〜120枚の画像枚数が通例であり、
1本目（fast_validation・約13.5分）にこの基準を単純に当てはめても60〜80枚程度が
妥当な水準になる。既存の `visual_pacing`（target_cut_sec等）をSCENE 002〜011にも
機械的に適用すると本動画は63カットとなり、この実務基準と整合することが判明した。
SCENE 001だけをカット分割し他を1画像/シーンに留める合理的な理由は薄いと判断した。

### 決定
SCENE 002〜011も、SCENE 001と同じ計算式・手順（`visual_pacing.target_cut_sec` と
`zone_overrides` に基づくカット数算出）でCUT形式へ変換した。P1/P2の `locked_prompt_fragment`、
該当ゾーンの `zone_color_grading`、確定スタイルタグ、`text_exclusion_guard`、該当ロケーション
（LOC01自宅リビング／LOC02銀行窓口）の `locked_prompt_fragment` を各CUTに反映している。
NARRATION本文（B-4承認対象）は一切変更していない。

ただし explanation（SCENE 009・010）・checklist（SCENE 011）ゾーンの計13カットについては、
P1/LOC01/LOC02のlocked_prompt_fragmentを機械的に全カットへ差し込むと、これらのゾーンが
意図する抽象的な説明図解・概念図（過信バイアスの象徴イラスト、チェックリストの
インフォグラフィック等）という設計意図と衝突するため、原則として抽象図解のまま維持する
方針とした。例外として、NARRATIONが物語内の具体的な場面（「健一が広告を見た、あの最初の夜」
「銀行の窓口に立った、あの朝」＝SCENE 009）に明示的に言及している箇所のみ、P1＋該当ロケーションを
用いた小さな回想（フラッシュバック）カットとして差し込んだ。SCENE 010は本ADRの既存決定
（一般化された人物像として意図的にP1を適用しない）を維持し、全カット抽象のままとした。

各SCENEのカット数（`visual_pacing`計算値通り）：
hook 7 / daily_life 4+4 / misjudgment_accumulation 6+6+6 / crisis_regret 8+9 /
explanation 5+6 / checklist 2 ＝ **合計63カット**。

### 結果
- `videos/001_sns_investment_scam/FINAL_SCRIPT_FULL.md` の全11シーンをCUT形式に統一し、
  メタブロックの `total_cuts` を63に更新した。
- `production_extractor.py` を再実行し、全11シーンのcut_pacing警告（修正前は10件）が
  すべて解消されたことを確認した。
- SCENE 009・011の一部カット、およびSCENE 010の全カットは、意図的にキャラクター/ロケーションの
  locked_prompt_fragmentを適用していない（上記「決定」参照）。今後同様の抽象図解ゾーンを扱う際は
  この判断基準（NARRATIONが具体的場面に言及する箇所のみ回想カットとして実キャラを使う）を
  踏襲する。
