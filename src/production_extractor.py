...
#
# 注意：現在は動画ごとのハードコード（002用の値）。001に対して再実行しても
# 機能しない。恒久対応（blueprint.json.visual_castのname/roleフィールドから
# 自動導出する設計）は検討済みだが未実装。詳細はセッション記録を参照
SPEAKER_KEYWORDS = {
    "拓也": ["川島拓也", "拓也"],
    "由紀": ["由紀", "姉"],
    "和彦": ["和彦", "叔父"],
...
# 002の和彦（叔父）はCUTプロンプト本文に役割を示す一意な語
# （001の"bank teller"に相当するもの）が無いため未登録
ROLE_SEARCH_TERMS = {}
